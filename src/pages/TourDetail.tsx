...
import { img, tourById } from '../data/tours'
  ...
  return (
    <>
      ...
      <div className="relative isolate overflow-hidden">
        <img src={tourPhoto(tour.id, tour.category) ?? img(`${tour.id}-hero`, 1600, 640)} alt={tour.name} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="shell relative py-20 sm:py-24">
          ...
          <span className="mt-4 block text-xs font-bold uppercase tracking-[0.2em] text-teal-100">{tour.region}</span>
          <h1 className="display mt-2 max-w-3xl text-4xl font-bold text-white drop-shadow sm:text-5xl">{tour.name}</h1>
          <p className="mt-4 max-w-xl text-lg text-white/85">{tour.summary}</p>
        </div>
      </div>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2">
            <div className="rounded-2xl bg-teal/5 p-6 ring-1 ring-teal/20">
              ...
              <ul className="mt-3 space-y-2 text-sm text-body">
                {tour.included.map((x) => <li key={x} className="flex gap-2"><span className="text-teal">•</span>{x}</li>)}
              </ul>
            </div>
            <div className="rounded-2xl bg-coral/5 p-6 ring-1 ring-coral/20">
              ...
              <ul className="mt-3 space-y-2 text-sm text-body">
                {tour.excluded.map((x) => <li key={x} className="flex gap-2"><span className="text-coral">•</span>{x}</li>)}
              </ul>
            </div>
          </div>
        </div>
          ...
          <div className="overflow-hidden rounded-2xl bg-white shadow-card ring-1 ring-line">
            ...
            <dl className="divide-y divide-line px-6 text-sm">
              ...
              <div className="py-3">
                ...
                <dd className="mt-1 font-medium text-navy">{tour.departurePoint}</dd>
              </div>
            </dl>
